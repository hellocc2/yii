...
class ProductLogic
{
		...
		function GetNewList($limit=2, $ishot = false, $cid = null)
	{
		...
		$sql = 'SELECT `id`,`name`,`flag`,`price`,`nowprice`,`img`, `linkid`,`virtualnum`,(`virtualnum`+`sells_count`) AS `sells_count`,
			`begintime`,`intro`,`type`,`overtime`,`maxnum`,`successnum`,`is_countdown`,`newbie_cut`,`client_cut`,`limit_level`,`rebate_money`
	...
	function GetOwnerList( $sellerID = 0, $limit = null, $parse = false, $productid = 0 ){
		...
		FROM `".table('product')."` WHERE `display`>0 AND `saveHandler`='normal' AND `sellerid`='".$sellerID."' AND `overtime`>=".time()." 
	...
	function GetList( $cid = -1, $actived = null, $extend = '1', $limit = null )
	{
		$cid = (int) $cid;
		$sql_limit_city = '1';
		if ( $cid > 0 )
