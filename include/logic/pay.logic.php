	...
	public function VirtualOrderTradeData($order = array(), $payment = false) {
		...
		if(false == $order) {
			return false;
				...
				$trade = $otrade;
		$trade['sign'] = $order['orderid'];
		$trade['price'] = $order['totalprice'];
		if(isset($trade['money'])) {
			$trade['money'] = ($otrade['price'] == $otrade['money'] ? $trade['price'] : $otrade['money']);
		}
				$trade['uid'] = $order['userid'];
		if(isset($trade['__order__'])) {
