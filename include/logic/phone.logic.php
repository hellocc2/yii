...
class PhoneLogic
{
	...
	public function Checkip($num = 15) {		
		$num = max(0, (int) $num);
		if($num > 0) {
			$ip = client_ip();
			if($ip) {
				$timezero = strtotime(date('Y-m-d',time()));
				$row = dbc()->query("SELECT COUNT(1) AS MDCS FROM ".table('push_log')." WHERE `userip`= '$ip' and `type`='sms' and `update`>=$timezero")->GetRow();
				if ($row['MDCS'] > $num) {
					 return  '注册次数超限！';
				}
			}
	...
	public function Check($phone, $check_exists = true, $check_ip = false) {
		if($check_ip && false != ($ret = $this->Checkip())) {
			return $ret;
		}
...
{$settings['site_url']}";
	...
	public function clear_invalid($uids = array()) {
		settype($uids, 'array');
		$query = dbc()->query("SELECT p.uid FROM ".table('phone')." p LEFT JOIN ".table('members')." m ON m.uid=p.uid WHERE m.uid IS NULL");
		while(false != ($row = $query->GetRow())) {
			$uids[$row['uid']] = $row['uid'];
		}
		if($uids) {
			dbc()->query("DELETE FROM ".table('phone')." WHERE uid IN ('".implode("','", $uids)."')");
		}
	}
