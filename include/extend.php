...
function page_moyo($sql = '')
{
	...
	if(is_numeric($sql)) {
		$total = (int) $sql;
				...
				$sql_count = preg_replace('/^\s*select\s.+?from/is', 'sELECt COUNT(1) aS MCNT fROm', $sql);
...
function admin_priv($priv_str, $uid = null)
{
	$uid = ((null === $uid) ? MEMBER_ID : (int) $uid);
	if($uid < 1) {
		return false;
	}
    	if($uid > 0 && true === AIJUHE_FOUNDER) {
		return true;
	}
		$user_priv = user($uid)->get('privs');
	if($user_priv && in_array(user($uid)->get('role_type'),array('admin','seller'))){
		if ($user_priv == 'all'){
			return true;
		}
...
function api_uid($appcode = '', $token = '', $apptoken = '') {
	$uid = 0;
	...
	if(empty($appcode) || empty($token)) {
		...
		if(empty($apptoken)) {
			$apptoken = handler('cookie')->GetVar('apptoken');
		}
	...
	if($appcode && $token) {
		...
		if(false != $row) {
			...
			handler('cookie')->SetVar('apptoken', $apptoken, 86400);
			$uid = $row['user_id'];
		}
	}
