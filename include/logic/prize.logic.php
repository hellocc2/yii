...
class PrizeLogic
{
	...
	public function bind($phone, $uid, $sync2user = false) {
		$row = dbc(DBCMax)->select('prize_phone')->where(array('uid'=>$uid, 'phone'=>$phone))->limit(1)->done();
		if($row) {
			dbc(DBCMax)->update('prize_phone')->where(array('id'=>$row['id']))->data(array('vftime'=>time()))->done();
		} else {
		...
		$sync2user && logic('phone')->bind($phone, $uid);
	}

	
	public function unbind($phone, $uid = 0, $sync2user = false) {
		$phone = (is_numeric($phone) ? $phone : 0);
		dbc(DBCMax)->delete('prize_phone')->where(array('phone' => $phone))->done();

		$uid = (is_numeric($uid) ? $uid : 0);
		if($uid > 0) {
			dbc(DBCMax)->delete('prize_phone')->where(array('uid' => $uid))->done();
		}
		
		$sync2user && logic('phone')->unbind($phone, $uid);
	}
