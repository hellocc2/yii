...
function referer($default = '?') {
	...
	if($referer=="" || 
		...
		strpos($referer,'account')!==false || 
		strpos($referer,'get_password')!==false || 
		(strpos($referer,":/"."/")!==false && 
			strpos($referer, preg_replace("~^www\.~",'',strtolower(getenv('HTTP_HOST') ? getenv('HTTP_HOST') : $_SERVER['HTTP_HOST'])))===false)) {
			if($default) {
...
function template($tpl_name) {
	return handler('template')->file($tpl_name);
}