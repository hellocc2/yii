...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['js_price'] 	 = price_format($row['market_price'] - $row['goods_price'], false);
		$row['give_integral'] 	 = $GLOBALS['db']->getOne("SELECT give_integral FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_id=".$row['goods_id']."");
		if($row['give_integral'] < 0){$row['give_integral'] = 0;}
		$total['goods_nuber'] += $row['goods_number'];
		$total['give_integral'] += ($row['give_integral']*$row['goods_number']);
