...
class MeLogic
{
	...
	{
		if(false == $ticket || TICK_STA_Unused != $ticket['status']) {
			return ;
		}
	...
	function growth($ov = 0){
        ...
        $html .= <<<EOF
当前等级： <font class='R16' ><b>VIP{$rets['level']}</b></font><br/>
当前成长值: {$rets['growth']}<br/>
距离下一等级：{$rets['distance']}<br/>
成长值 = 购买金额 - 退款金额
EOF;
		if($ov){
			echo '<b>VIP'.$rets['level'].'</b>';
		}else{
			echo $html;
		}
    }
    ...
    function level($uid = null) {
    	$rets = array();
    	$uid = (int) (null === $uid ? user()->get('id') : $uid);
    	if($uid > 0) {
    		    		$growth = (int) user($uid)->get('totalpay');
    		$levels = explode(',', ini('credits.level'));
    		
    		$level = 0;
    		for ($i=1; $i<=count($levels); $i++) {
    			if($growth >= $levels[$i-1]) {
    				$level = $i;
    			}
    		}
    		$distance = $levels[$level] - $growth;
    		$distance < 0 && $distance = 0;

    		$rets = array(
    			'level' => $level,     			'growth' => $growth,     			'distance' => $distance,     		);
    	}
    	return $rets;
    }
}
