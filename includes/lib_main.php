...
function get_shop_help()
{
    ...
    foreach ($res AS $key => $row)
    {
        ...
        $arr[$row['cat_id']]['article'][$key]['url']         = $row['open_type'] != 1 ?
            'help.php?id='.$row['article_id'] : trim($row['file_url']);
			
       // $arr[$row['cat_id']]['article'][$key]['url']         = 'help.php?id='.$row['article_id'];			
...
{
    assign_template();
	$GLOBALS['smarty']->assign('isclass',          'no');
    $msg['content'] = $content;
 ...
 * @return  array
 */
function assign_comment($id, $type, $page = 1 , $cls=0)
{
    /* 取得评论列表 */
	if($cls == 1){
		$addsql = " and comment_rank in(1,2)";
		$clsname= '差评';
	}elseif($cls == 2){
		$addsql = " and comment_rank in(3)";
		$clsname= '中评';
	}elseif($cls == 3){
		$addsql = " and comment_rank in(4,5)";
		$clsname= '好评';
	}else{
		$addsql = "";
		$clsname= '评论';
    ...
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0".$addsql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['comment_id']]['clsname']  = $clsname;
    ...
    $pager['page_count']   = $page_count;
    $pager['page_first']   = "javascript:gotoPage(1,$id,$type,$cls)";
    $pager['page_prev']    = $page > 1 ? "javascript:gotoPage(" .($page-1). ",$id,$type,$cls)" : 'javascript:;';
    $pager['page_next']    = $page < $page_count ? 'javascript:gotoPage(' .($page + 1) . ",$id,$type,$cls)" : 'javascript:;';
    $pager['page_last']    = $page < $page_count ? 'javascript:gotoPage(' .$page_count. ",$id,$type,$cls)"  : 'javascript:;';
	
	$pager['plall']    	=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('comment') . " where id_value = ".$id);
	$pager['pl1']    	=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('comment') . " where comment_rank in(4,5) and id_value = ".$id);
	$pager['pl2']    	=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('comment') . " where comment_rank in(3) and id_value = ".$id);
	$pager['pl3']    	=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('comment') . " where comment_rank in(1,2) and id_value = ".$id);
