        	...
        	if(is_array($orders) && count($orders)) {        		       		
        		foreach($orders as $order) {
        			$status = $status_default;
        			        			...
        			        			if(in_array($status, array('WAIT_SELLER_SEND_GOODS', )) && 
...
class Order_Bridge_CallbackLogic extends CallbackLogic
{
	...
	public function SrcOne($orderid, $more = false)
	{
		if($more) {
			return logic('order')->GetOne($orderid);
		} else {
...
class Recharge_Parser_CallbackLogic extends CallbackLogic
{
	...
	{
				$trade = logic('pay')->TradeData($payment);
		if($trade['sign'] && true === (logic('recharge')->MakeSuccessed($trade['sign']))) {
			if(logic('pay')->Process($payment, 'TRADE_FINISHED')) return;
...
class Recharge_Bridge_CallbackLogic extends CallbackLogic
{
	...
	public function SrcOne($orderid, $more = null)
	{
...
{
	public function SrcOne($sign, $more = null)
	{
		return array();
	}
}
