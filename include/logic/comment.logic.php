...
class CommentManageLogic
{
	...
	public function front_user_submit($order_id, $score, $content, $user_id = null, $comment_id = null, $img = '', $anonymous = 0, $product_id = 0, $virtual_username = '')
	{
		if ((int)$score > 0 && $content)
		{
			...
			if($product_id > 0 && true === admin_priv('virtual_comments', $user_id)) {
				if($virtual_username) {
					$user_name = $virtual_username;
				}
				$status = "approved";
			} else {
	...
	public function admin_vlist($where = ' 1 ')
	{
		$sql = dbc(DBCMax)->select('comments')->where($where)->order('timestamp_update.desc')->sql();
		$sql = page_moyo($sql);
		$comments = dbc(DBCMax)->query($sql)->done();
		$comments || $comments = array();
		$products = array();
