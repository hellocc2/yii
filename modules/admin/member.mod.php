...
class ModuleObject extends MasterObject
{
	...
	function DoModify()
	{
		...
		$userinfo = dbc(DBCMax)->query('select uid,username,`email`,`password`,phone,role_id,role_type,privs,money,email2 from '.table('members').' where uid='.(int) $this->Post['uid'])->limit(1)->done();
		if(!$userinfo){
		...
		if($phone != $userinfo['phone']) {
				...
				$home_uid = (int) $this->Post['home_uid'];
		$this->Post['home_uid'] = (($home_uid > 0 && $home_uid != $userinfo['uid'] && $home_uid == user($home_uid)->get('id')) ? $home_uid : 0);
