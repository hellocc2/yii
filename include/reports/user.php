...
class reports_unit_user
{
	...
	private function get_first_user_time($hoster)
	{
		$where = " paytime > '0' ";
		if($hoster) {
			$where .= " AND `userid`='{$hoster}' ";
		}

		$order = dbc(DBCMax)->select('order')->where($where)->order('paytime.asc')->limit(1)->done();
		$recharge = dbc(DBCMax)->select('recharge_order')->where($where)->order('paytime.asc')->limit(1)->done();
