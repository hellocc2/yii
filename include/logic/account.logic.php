...
class AccountLogic
{
	...
	public function Register($username, $password, $mail = '', $phone = '', $qq = '', $noExRegister = false, $home_uid=0)
	{
		...
		if (!$iid)
		{
			return $this->ErrorInf('注册失败！（本地数据库错误）');
				...
				$this->rebate($iid, ini('member.register.rebate'));
				$data['password'] = $password;
		logic('notify')->Call($iid, 'logic.account.register.done', $data);
				return $this->SuccInf($iid);
	}
	
	public function rebate($uid, $rebate = 0) {
		$uid = (is_numeric($uid) ? $uid : 0);
		$rebate = (float) round($rebate, 2);
		if($uid > 0 && $rebate > 0 && user($uid)->get('id') > 0) {
			logic('me')->money()->add($rebate, $uid, array(
				'name' => '注册红包',
				'intro' => '恭喜您，用户新注册送红包<b>&yen;'.$rebate.'元</b>'
			));
		}
	}
	...
	public function exLogin($username, $password)
	{
...
class AccountLogic_uLogin
{
	...
	public function reg_and_login($from)
	{
		...
		if(false == $ret) {
			$row = dbc()->Query("SELECT `username`, `password` FROM ".table('members')." WHERE `email2`='{$unique_key}' LIMIT 1")->GetRow();

			$loginR = account()->Login($row['username'], $row['password']);

			$ret = ($loginR['error'] ? false : true);
		}
