	...
	private function coupon_used($number, $password, $morecoupon = '')
	{
		...
		else
		{
			if(isset($result['coupon']['number']) && isset($result['coupon']['password']) && $number == $result['coupon']['number']) {
				...
				exit('<font color="green coupon_used_qun_tag">已成功消费 </font><font color="red">'.count($result['coupon']).'</font><font color="green"> 张' . TUANGOU_STR . '券：'.implode(',',$result['coupon']).'</font>|||success');
			}
		}
	}
