...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'get_pass', 'pointslist');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'get_pass', 'pointslist');
...
if ($action == 'default')
{
    ...
    $orders = get_user_orders($user_id, 5, $pager['start']);
...
elseif ($action == 'pointslist')
{

	include_once(ROOT_PATH .'includes/lib_clips.php');
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('account_log'). " WHERE user_id = '$user_id' and change_type = '99'");
    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);
    $sql = "SELECT * FROM " .$ecs->table('account_log'). " WHERE user_id = '$user_id' and change_type = '99'";
    $list = $GLOBALS['db']->getAll($sql, $pager['size'], $pager['start']);
	foreach($list as $v=>$k){
		$list[$v]['change_time'] = date('Y-m-d H:i:s',$k['change_time']);
	}
    $smarty->assign('pager',  $pager);
    $smarty->assign('pointslist', $list);
    $smarty->assign('back_act', $back_act);
	$smarty->assign('info',        get_user_default($user_id));
    $smarty->display('user_clips.dwt');
