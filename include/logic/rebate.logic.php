...
class RebateLogic
{
	...
	public function Add_Rebate_For_Ticket($product, $fix_time = false){
		$sellerid = $product['sellerid'];
		$orderid = $product['coupon']['orderid'];
		$ticketid = $product['coupon']['ticketid'];
		if($fix_time) {
			$this->time = ($product['coupon']['usetime'] > 0 ? strtotime($product['coupon']['usetime']) : time());
		}
	...
	public function Add_Rebate_For_Item($data_order, $fix_time = false){
		...
		$userid = $data_order['userid'];
		if($fix_time) {
			$this->time = max($data_order['paytime'], $data_order['process_time']);
		}
	...
	public function get_my_rebate_list($where = ''){
		$sql = "SELECT r.*,t.number,t.mutis,p.flag FROM ". table('rebate_log') ." r
			...
			WHERE r.uid='". MEMBER_ID . "' AND r.type='master' ". $where ." ORDER BY r.id DESC";
		$sql = page_moyo($sql);
		$res = dbc(DBCMax)->query($sql)->done();
		if( $res ){
			return $res;
