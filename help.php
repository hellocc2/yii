...
$_REQUEST['id'] = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 9;
$article_id     = $_REQUEST['id'];
if(isset($_REQUEST['cat_id']) && $_REQUEST['cat_id'] < 0)
{
    //$article_id = $db->getOne("SELECT article_id FROM " . $ecs->table('article') . " WHERE cat_id = '".intval($_REQUEST['cat_id'])."' ");
	$helparr 	= get_shop_help();
	$temparr 	= array_slice(get_shop_help(),1);
	$temparr 	= $temparr[0]['article'];
	$temparr 	= array_slice($temparr,1);
	$article_id = $temparr[0]['article_id'];
}
...
if (!$smarty->is_cached('help.dwt', $cache_id))
{
	...
	$smarty->assign('index_style',     'curr'); 
    ...
    {
        $next_article['url'] = 'help.php?id='.$next_article['article_id'];
    ...
    if (!empty($prev_aid))
    {
        ...
        $prev_article['url'] = 'help.php?id='.$prev_article['article_id'];
