...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, consignee, goods_amount, pay_name, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
    	...
    	$goodslist = $GLOBALS['db']->GetAll("SELECT goods_id FROM ".$GLOBALS['ecs']->table('order_goods') ." WHERE order_id = ".$row['order_id']."");
		foreach($goodslist as $k=>$v){
			$goodslist[$k]['goods_thumb']	=  $GLOBALS['db']->GetOne("SELECT goods_thumb FROM ".$GLOBALS['ecs']->table('goods') ." WHERE goods_id = ".$v['goods_id']."");
			$goodslist[$k]['goods_name'] 	=  $GLOBALS['db']->GetOne("SELECT goods_name FROM ".$GLOBALS['ecs']->table('goods') ." WHERE goods_id = ".$v['goods_id']."");
			$goodslist[$k]['url'] 		  	= build_uri('goods', array('gid'=>$v['goods_id']), $goodslist[$k]['goods_name']);
		}
