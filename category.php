...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update', 'salesnum', 'commentnum'))) ? trim($_REQUEST['sort'])  : 'salesnum';
//$sort = $_REQUEST['sort'];
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
	...
	$tempinfo = $db->getRow('SELECT * FROM '.  $ecs->table('category') ." WHERE cat_id = '$cat_id'");
	if($tempinfo['parent_id'] == '0'){
		$pid = $tempinfo['cat_id'];
	}else{
		$pid = $tempinfo['parent_id'];
		$catinfo2 = $db->getRow('SELECT * FROM '.  $ecs->table('category') ." WHERE cat_id = '$cat_id'");
	}
	$catinfo1 = $db->getRow('SELECT * FROM '.  $ecs->table('category') ." WHERE cat_id = '$pid'");
	$smarty->assign('catinfo1',    $catinfo1);
	$smarty->assign('catinfo2',    $catinfo2);
