...
class OrderLogic
{
	...
	public function GetList( $uid = 0, $status = ORD_STA_ANY, $pay = ORD_PAID_ANY, $extend = '1',$notpage = 1 )
	{
		...
		$sql_limit_user = '1 and o.status != '.ORD_STA_Virtual.' and o.status != '.ORD_STA_Virtual_AlREDAY;
		if ($uid > 0)
		{
			$sql_limit_user = 'o.userid = '.$uid.' and o.status != '.ORD_STA_Virtual.' and o.status != '.ORD_STA_Virtual_AlREDAY;;
		}
		$sql_limit_status = '1';
		if ( $status >= 0 )
		{
			$sql_limit_status =  'o.status = '.$status;
		}
		$sql_limit_pay = '1';
		if ( $pay >= 0 )
		{
			$sql_limit_pay = 'o.pay = '.$pay .' AND o.process<>"_TimeLimit_" ';
		}
		$sql = 'SELECT o.*, m.username, s.userid AS sellerid
		FROM
			' . table('order') .' o
		LEFT JOIN
			' .table('members'). ' m
		...
		if($vorder) {
			...
			if(is_array($orders) && $orders_count > 0) {
				...
				foreach($orders as $order) {
										if($order['paytype'] != $vorder['paytype'] || $order['paymoney'] != $order['totalprice']) {
	...
	public function _TMP_Payed($trade, $payment = false)
	{
		...
		if ($order['pay'] == ORD_PAID_No)
		{
						...
						if (!$trade['nmpay'])
			{
				...
				if(($price_cut=logic('cut')->order_calc($order['orderid'], $price))>0) {
					$cutss = sprintf(__('优惠立减：%.2f<br>'), $price_cut);
				}
				...
				else
				{
					...
					elseif ($money >= $price)
					{
						...
						{
							$express = sprintf(__('运费：%.2f<br>'), $order['expressprice']);
						}
						#钱的减少
						logic('me')->money()->pay($price, $order['userid'], array(
							'name' => __('购买商品'),
							'intro' => sprintf(__('订单号：%s<br/>商品名：%s<br/>单价：%.2f<br/>数目：%d<br/>%s %s'), $order['orderid'], $order['product']['flag'], $order['productprice'], $order['productnum'], $express, $cutss)
						));
												$order_update = array(
							'pay' => ORD_PAID_Yes,
							'paytime' => time()
						...
						if($price > 0) {
														$this->rebate_buy($order, $price);

														$this->rebate_reg($order, $price);
						}
					}
					else
					{
		...
		private function rebate_reg($order, $price = null) {
		$uid = (int) $order['userid'];
		$reg_money = ini('rebate_setting.reg_money');
		$price = (float) (null === $price ? $order['paymoney'] : $price);
		if($uid > 0 && $reg_money > 0 && $price > 0) {
			$home_uid = user($uid)->get('home_uid');
			if($home_uid > 0 && $this->Count(" `userid`='{$uid}' AND `paymoney`>'0' AND `pay`='1' ") < 2) {
		...
		private function rebate_buy($order, $price = null) {
		$uid = (int) $order['userid'];
		$price = (float) (null === $price ? $order['paymoney'] : $price);
		if($uid < 1 || $price <= 0) {
			return ;
		...
		if($rebate_money < 0) {
			...
			if($rebate_limit > 0) {
								if($this->Count(" `userid`='{$uid}' AND `productid`='{$order['productid']}' AND `pay`='1' ") <= $rebate_limit) {
					$money = $rebate_money;
				}
			} else {
				$money = $rebate_money;
	...
	public function MakeSuccessed($orderid, $ignore_rules = false)
	{
		...
		logic('notify')->Call($order['userid'], 'logic.order.MakeSuccessed', $notify_data);
		if ('stuff' == $order['product']['type']) {
			$address = logic('address')->GetOne($order['addressid']);
			$notify_data['address'] = $address['region'] . $address['address'];
			$notify_data['address_name'] = $address['name'];
			$notify_data['address_phone'] = $address['phone'];
			notify($order['product']['selleruserid'], 'remind.seller.delivery', $notify_data);
		}
		meta($MID, time(), 'd:30');
		return $order;
	}
	...
	private function __parse_result($data)
	{
		...
		$data['cuts'] = logic('cut')->snapshot($data['orderid']);
	...
	public function Confirm($oid)
	{
		...
		if (!$error)
		{
			logic('order')->Processed($oid, '__PAY_YET__');
			logic('order')->clog($oid)->sys('管理员后台手工确认付款：用户已付款', '__PAY_YET__');
