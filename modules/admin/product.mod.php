...
class ModuleObject extends MasterObject
{
			...
			if (is_numeric($prosta)) {
				switch ($prosta) {
					...
					case PRO_STA_WillExpire:							$whereSql .= ' AND p.overtime >= ' . time();
						...
						break;
					case PRO_STA_Expired:								$whereSql .= ' AND p.overtime < ' . time();
						$orderSql .= ' p.overtime DESC,';
						break;
					case PRO_STA_Alarm:									$whereSql .= ' AND `overtime` >= '. time() .'  AND p.maxnum > p.sells_count AND p.maxnum - p.sells_count < 10';
						break;
	...
	function Save()
	{
		...
		if('admin' == MEMBER_ROLE_TYPE) {
			...
			if($data['newbie_cut'] > $data['nowprice'] || $data['client_cut'] > $data['nowprice']) {
