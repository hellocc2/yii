...
class PushLogic
{
	...
	public function ListLogCheck($type = null, $exsql = false, $addsql=null)
	{
		$sql_limit_type = '1';
		if ($type !== null)
		{
		...
		$sql = 'SELECT * FROM '.table('push_log').' WHERE '.$sql_limit_type.' AND '.($exsql ? $exsql : '1').$addsql.' ORDER BY id DESC';
		$sql = page_moyo($sql);
		$r = dbc(DBCMax)->query($sql)->done();
		if ($r)
		{
