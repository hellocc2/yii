...
class FundLogic
{
	...
	public function Orderdone($orderid,$status,$info)
	{
		...
		if (!$order || $order['status'] != 'doing')
		{
			...
			notify($order['userid'], 'logic.fund.orderdone', array(
					'username' => user($order['userid'])->get('name'),
					'orderid' => $orderid,
					'money' => $order['money'],
					'status' => ('yes' == $status ? '结算成功' : '结算失败'),
				));
		}
		return $return;
	}
	...
	public function get_last_log($orderid) {
		...
		return dbc(DBCMax)->select('fund_order_log')->where(array('orderid'=>$orderid))->order(' `id` DESC ')->limit(1)->done();
	}
}
?>